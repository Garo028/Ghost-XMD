...
async function startSocket(entry, sessionDir) {
    try {
        ...
        // ── Request a pairing code shortly after the socket is created ────────
        // entry.pairingRequested (not a local var) so this survives the
        // reconnect that follows code entry — otherwise a reconnect would
        // request (and silently swap in) a brand new code out from under the
        // user while they're typing it.
        // NOTE: matches the proven pattern used elsewhere (ScottyTech) — a
        // flat delay after socket creation, not hooked to a connection.update
        // state, which was landing at the wrong point in the handshake.
        if (!sock.authState.creds.registered && !entry.pairingRequested) {
            (async () => {
                await delay(2000);
                if (entry.pairingRequested) return; // already handled by a retry/reconnect
        ...
        sock.ev.on('connection.update', async ({ connection, lastDisconnect }) => {
            ...
            if (connection === 'close') {
                ...
                if (loggedOut) {
                    ...
                    // Same flat delay for every non-logout disconnect (including
                    // the restart WA triggers right after code entry) — matches
                    // the proven pattern elsewhere; reconnecting with zero delay
                    // here was racing WhatsApp's own server-side state.
