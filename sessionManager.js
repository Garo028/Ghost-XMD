...
async function createSession(number) {
    ...
    if (existing) {
        if (existing.status === 'connected' || existing.status === 'code_ready') {
            return existing;
        }
        const stuck = existing.status === 'pairing' &&
            (Date.now() - existing.createdAt) > STUCK_PAIRING_MS;
        if (existing.status === 'pairing' && !stuck) {
            // still within its normal window — let it finish instead of
            // spawning a second socket for the same number.
            return existing;
        }
        // Stuck in 'pairing' past the timeout, or previously errored/logged
        // out — tear it down and start clean.
        destroySession(id);
    }
